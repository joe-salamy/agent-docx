...
import { MdPageCountError } from "../dist/index.js";
...
test("LibreOffice terminates runaway transport output", async () => {
  const root = await mkdtemp(join(tmpdir(), "md-page-count-overflow-lo-"));
...
test("LibreOffice adapter uses isolated exact conversion arguments and PDF page tree", async () => {
  ...
  try {
    ...
    delete process.env.MD_PAGE_COUNT_FAKE_PDF;
    await rm(root, { recursive: true, force: true });
  }
});

test("LibreOffice renders the exact returned generated DOCX", async () => {
  const root = await mkdtemp(join(tmpdir(), "md-page-count-fake-lo-sha-"));
  const executable = join(root, "soffice");
  const pdf = await PDFDocument.create();
  pdf.addPage();
  process.env.MD_PAGE_COUNT_FAKE_PDF = Buffer.from(await pdf.save()).toString(
  ...
  const script = `#!/usr/bin/env node\nimport {writeFileSync} from 'node:fs';import {join} from 'node:path';const a=process.argv.slice(2);if(a[0]==='--version'){console.log('LibreOffice 99.0 fake');process.exit(0)}const out=a[a.indexOf('--outdir')+1];writeFileSync(join(out,'render.pdf'),Buffer.from(process.env.MD_PAGE_COUNT_FAKE_PDF,'base64'));`;
  try {
    await writeFile(executable, script);
    await chmod(executable, 0o755);
...
test("Word bridge validates and maps version-2 Unicode paragraph records", async () => {
  const root = await mkdtemp(join(tmpdir(), "md-page-count-fake-word-"));
...
test("Word paragraph extraction failure preserves a successful summary", async () => {
  const root = await mkdtemp(join(tmpdir(), "md-page-count-fake-word-error-"));
  const executable = join(root, "powershell");
  const script = `#!/usr/bin/env node
...
test("Word bridge rejects malformed frame ordering and framing", async () => {
  const root = await mkdtemp(join(tmpdir(), "md-page-count-fake-word-bad-"));
...
test(
  ...
  { skip: process.env.MD_PAGE_COUNT_WORD_SMOKE !== "1" },
  async () => {
