...
test("LibreOffice resolver rejects an explicit missing executable",async()=>{
  await assert.rejects(()=>resolveLibreOffice("/definitely/missing/soffice"),error=>error instanceof MdPageCountError&&error.code==="LIBREOFFICE_NOT_FOUND");
});
...
test("LibreOffice adapter uses isolated exact conversion arguments and PDF page tree",async()=>{
  ...
  process.env.MD_PAGE_COUNT_FAKE_PDF=Buffer.from(await pdf.save()).toString("base64");
  ...
  }finally{delete process.env.MD_PAGE_COUNT_FAKE_PDF;await rm(root,{recursive:true,force:true})}
});
