...
import { builtInProfiles, estimateMarkdown, inspectDocxTemplate, measureMarkdown, MdPageCountError } from "../dist/index.js";
...
test("unsupported Markdown rejects with source-aware code",async()=>{
  await assert.rejects(()=>estimateMarkdown("| A |\n| - |\n| x |"),error=>error instanceof MdPageCountError&&error.code==="UNSUPPORTED_MARKDOWN");
  await assert.rejects(()=>estimateMarkdown("```js\nalert(1)\n```"),error=>error instanceof MdPageCountError&&error.code==="UNSUPPORTED_MARKDOWN");
});
