...
import { AgentDocxError, type CliErrorPayload, type JsonValue } from "./types.js";
...
const parse = (
  ...
  const parsed = parseArgs({ args: [...args], options, strict: true, allowPositionals: true, tokens: true });
  const seen = new Set<string>();
  for (const token of parsed.tokens) {
    if (token.kind !== "option" || !token.name) continue;
    if (seen.has(token.name))
      throw new AgentDocxError("INVALID_ARGUMENT", `Duplicate option: --${token.name}`);
    seen.add(token.name);
  }
...
const noPositionals = (parsed: Parsed, label: string): void => {
  if (parsed.positionals.length > 0)
    throw new AgentDocxError("INVALID_ARGUMENT", `${label} does not accept positional arguments`);
};

const readJson = async (cwd: string, path: string, label: string): Promise<unknown> => {
  try {
...
const projectOption = (values: Values): string | undefined => optional(values, "project");

const documentId = async (cwd: string, values: Values): Promise<string> => {
  const explicit = optional(values, "document");
  if (explicit) return explicit;
  const project = await openProject(resolve(cwd, projectOption(values) ?? "agent-docx.json"));
...
): AgentRequest => ({ schemaVersion: 1, action, ...(project ? { project } : {}), params });
const print = async (runtime: CliRuntime, json: boolean, value: unknown): Promise<void> => {
  const serializable = serializeAgentValue(value, runtime.cwd);
  await runtime.writeStdout(`${JSON.stringify(serializable, null, json ? 0 : 2)}\n`);
};
...
const parsePatchArgs = async (
  ...
  if (patch && (base || edits))
    throw new AgentDocxError("INVALID_ARGUMENT", "Use --patch or --base with --edits, not both");
  if (patch) return readJson(cwd, patch, "patch");
  if (!base || !edits)
    throw new AgentDocxError("INVALID_ARGUMENT", "draft command requires --patch or both --base and --edits");
  return {
    schemaVersion: 1,
...
const inputFromProjectFlags = async (cwd: string, values: Values) => {
  const metadata = await readJson(cwd, required(values, "metadata"), "metadata");
  ...
  if ((fontFamily === undefined) !== (fontRegular === undefined))
    throw new AgentDocxError("INVALID_ARGUMENT", "--font-family and --font-regular must be supplied together");
  return {
    ...
    ...(optional(values, "template") ? { template: optional(values, "template") } : {}),
    ...(optional(values, "assets-dir") ? { assetsDir: optional(values, "assets-dir") } : {}),
    ...(optional(values, "filing-kind") ? { filingKind: optional(values, "filing-kind") } : {}),
    ...(optional(values, "rule-pack") ? { rulePack: optional(values, "rule-pack") } : {}),
    ...(fontFamily && fontRegular
      ? {
          fontSet: {
            family: fontFamily,
            regularPath: fontRegular,
            ...(optional(values, "font-bold") ? { boldPath: optional(values, "font-bold") } : {}),
            ...(optional(values, "font-italic") ? { italicPath: optional(values, "font-italic") } : {}),
...
type WatchDependencyState = {
  entries: readonly { key: string; path: string; byteLength: number; sha256: string }[];
...
const watchDependencies = async (
  ...
  const config = state.manifest.documents.find((document) => document.id === documentId);
  if (!target || !config)
    throw new AgentDocxError("DOCUMENT_NOT_FOUND", `Document not found: ${documentId}`);

  const entries: { key: string; path: string; byteLength: number; sha256: string }[] = [];
  const paths = new Set<string>([manifestPath]);
  const add = async (key: string, path: string, sha256: string): Promise<void> => {
    paths.add(path);
    entries.push({
      key,
  ...
  await add("source", resolve(projectDirectory, config.source), target.sourceSha256);
  if (config.template)
    await add(
      "template",
  ...
  if (config.assetsDir) {
    ...
    for (const [key, sha256] of Object.entries(target.dependencyObjects)
      ...
      if (
        ...
        throw new AgentDocxError("PROJECT_INVALID", `Invalid asset dependency: ${key}`);
...
const runAgentWatch = async (
  ...
  const refreshWatchedPaths = async (): Promise<void> => {
    const next = await watchDependencies(runtime.cwd, projectPath, targetDocument);
...
export const runWorkflowCommand = async (
  ...
  switch (command.command) {
    case "project": {
      ...
      if (subcommand === "add" && parsed.values.default === true) params.makeDefault = true;
      action = subcommand === "init" ? "project.init" : "project.add";
      break;
    }
    case "document": {
      parsed = parse(rest, { ...commonProject, document: string, base: string, changes: string, author: string, email: string, message: string });
      ...
      params = {
        ...
        changes: await readJson(runtime.cwd, required(parsed.values, "changes"), "changes"),
    ...
    case "revision": {
      if (subcommand === "checkpoint") {
        parsed = parse(rest, { ...commonProject, document: string, base: string, author: string, email: string, message: string });
        noPositionals(parsed, "revision checkpoint");
        params = { documentId: await documentId(runtime.cwd, parsed.values), baseRevision: optional(parsed.values, "base") ?? null, author: actor(parsed.values), message: required(parsed.values, "message") };
        action = "revision.checkpoint";
      } else if (subcommand === "list") {
        parsed = parse(rest, { ...commonProject, document: string, limit: string, cursor: string });
        noPositionals(parsed, "revision list");
        const limit = optional(parsed.values, "limit");
        params = { documentId: await documentId(runtime.cwd, parsed.values), ...(limit ? { limit: Number(limit) } : {}), ...(optional(parsed.values, "cursor") ? { cursor: optional(parsed.values, "cursor") } : {}) };
        action = "revision.list";
      } else if (subcommand === "show") {
        parsed = parse(rest, { ...commonProject, document: string, revision: string });
        if (parsed.positionals.length > 1) throw new AgentDocxError("INVALID_ARGUMENT", "revision show accepts one revision");
        params = { documentId: await documentId(runtime.cwd, parsed.values), revision: optional(parsed.values, "revision") ?? parsed.positionals[0] };
        action = "revision.get";
      } else if (subcommand === "restore") {
        parsed = parse(rest, { ...commonProject, document: string, base: string, author: string, email: string, message: string });
        if (parsed.positionals.length !== 1) throw new AgentDocxError("INVALID_ARGUMENT", "revision restore requires a target revision positional");
        params = { documentId: await documentId(runtime.cwd, parsed.values), baseRevision: required(parsed.values, "base"), targetRevision: parsed.positionals[0], author: actor(parsed.values), message: required(parsed.values, "message") };
        action = "revision.restore";
      } else if (subcommand === "diff") {
        parsed = parse(rest, { ...commonProject, document: string, base: string, head: string, output: string });
        if (parsed.positionals.length > 2) throw new AgentDocxError("INVALID_ARGUMENT", "revision diff accepts BASE and HEAD positionals");
        params = {
          documentId: await documentId(runtime.cwd, parsed.values),
          baseRevision: optional(parsed.values, "base") ?? parsed.positionals[0],
          headRevision: optional(parsed.values, "head") ?? parsed.positionals[1],
          ...(optional(parsed.values, "output") ? { output: optional(parsed.values, "output") } : {}),
        };
        action = "revision.diff";
      } else {
        parsed = parse(rest, { ...commonProject, document: string, "change-set": string, decisions: string, author: string, email: string, message: string });
        noPositionals(parsed, "revision resolve");
        params = {
          documentId: await documentId(runtime.cwd, parsed.values),
          changeSet: await readJson(runtime.cwd, required(parsed.values, "change-set"), "change set"),
          decisions: await readJson(runtime.cwd, required(parsed.values, "decisions"), "decisions"),
          author: actor(parsed.values),
          message: required(parsed.values, "message"),
        };
    ...
    case "draft": {
      if (subcommand === "guidance") {
        ...
        params = { documentId: await documentId(runtime.cwd, parsed.values), ...(optional(parsed.values, "revision") ? { revision: optional(parsed.values, "revision") } : {}) };
        action = "draft.guidance";
      } else {
        parsed = parse(rest, { ...commonProject, document: string, patch: string, base: string, edits: string, renderer: string, "patch-hash": string, gate: string, author: string, email: string, message: string });
        noPositionals(parsed, `draft ${subcommand}`);
        const targetDocument = await documentId(runtime.cwd, parsed.values);
        params = { patch: await parsePatchArgs(runtime.cwd, parsed.values, targetDocument), ...(optional(parsed.values, "renderer") ? { renderer: optional(parsed.values, "renderer") } : {}) };
        if (subcommand === "apply") {
          params.patchHash = required(parsed.values, "patch-hash");
          if (optional(parsed.values, "gate")) params.gate = optional(parsed.values, "gate");
    ...
    case "review": {
      if (subcommand === "add") {
        parsed = parse(rest, { ...commonProject, document: string, revision: string, block: string, start: string, end: string, author: string, email: string, message: string });
        noPositionals(parsed, "review add");
        const start = optional(parsed.values, "start");
        const end = optional(parsed.values, "end");
        if ((start === undefined) !== (end === undefined)) throw new AgentDocxError("INVALID_ARGUMENT", "--start and --end must be supplied together");
        params = { documentId: await documentId(runtime.cwd, parsed.values), revision: required(parsed.values, "revision"), blockId: required(parsed.values, "block"), ...(start && end ? { range: { start: Number(start), end: Number(end) } } : {}), author: actor(parsed.values), message: required(parsed.values, "message") };
        action = "review.add";
      } else {
        parsed = parse(rest, { ...commonProject, document: string, revision: string, annotation: string, author: string, email: string, message: string });
        noPositionals(parsed, "review resolve");
        params = { documentId: await documentId(runtime.cwd, parsed.values), revision: required(parsed.values, "revision"), annotationId: required(parsed.values, "annotation"), author: actor(parsed.values), message: required(parsed.values, "message") };
        action = "review.resolve";
      }
      break;
    }
    case "validate": {
      parsed = parse(command.args, { ...commonProject, document: string, revision: string });
    ...
    case "export": {
      parsed = parse(command.args, { ...commonProject, document: string, revision: string, mode: string, base: string, output: string, renderer: string, "office-timeout": string, "libreoffice-path": string });
      ...
      params = {
        ...
        ...(optional(parsed.values, "base") ? { baseRevision: optional(parsed.values, "base") } : {}),
        ...((renderer || officeTimeoutMs || libreofficePath)
          ? { options: { ...(renderer ? { renderer } : {}), ...(officeTimeoutMs ? { officeTimeoutMs: Number(officeTimeoutMs) } : {}), ...(libreofficePath ? { libreoffice: { executablePath: libreofficePath } } : {}) } }
          : {}),
      };
      action = "docx.export";
      break;
  ...
  const result = await dispatchAgentRequest(request(action, projectOption(parsed!.values), params!), runtime.cwd);
