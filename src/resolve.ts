...
import { MdPageCountError, type Diagnostic, type EstimateOptions, type FontSetInput, type LayoutProfile, type MetricFont, type ResolvedLayoutProfile, type TextStyle } from "./types.js";
...
export function resolveProfile(options:EstimateOptions):LayoutProfile{
  ...
  if(typeof options.profile==="string"){
    ...
    if(!selected)throw new MdPageCountError("INVALID_ARGUMENT",`Unknown profile: ${options.profile}`);
...
export async function loadFonts(input:FontSetInput|undefined,requested:string):Promise<LoadedFonts>{
  ...
  if("fonts" in createdRegular)throw new MdPageCountError("INVALID_FONT","Font collections are not accepted; supply a single font face");
  const regularFont:Font=createdRegular;
  const embedded=regularFont.familyName??source.family;
  if(input&&embedded.toLocaleLowerCase("en-US")!==source.family.toLocaleLowerCase("en-US"))throw new MdPageCountError("INVALID_FONT",`Regular font family ${embedded} does not match ${source.family}`);
  ...
  for(const role of roles){
    ...
    if("fonts" in created)throw new MdPageCountError("INVALID_FONT",`${role} is a font collection; supply a single font face`);
    const font:Font=created;
    const family=font.familyName??embedded;
    if(input&&family.toLocaleLowerCase("en-US")!==embedded.toLocaleLowerCase("en-US"))throw new MdPageCountError("INVALID_FONT",`${role} face family ${family} differs from regular face ${embedded}`);
