...
const assertKeys = (
  ...
  if (unknown)
    throw new AgentDocxError("INVALID_ARGUMENT", `${label} has unknown property: ${unknown}`);
};
...
const optionalString = (value: Record<string, unknown>, key: string): string | undefined => {
  ...
  if (typeof value[key] !== "string" || value[key] === "")
    throw new AgentDocxError("INVALID_ARGUMENT", `${key} must be a non-empty string`);
  return value[key] as string;
};

const optionalBoolean = (value: Record<string, unknown>, key: string): boolean | undefined => {
  if (value[key] === undefined) return undefined;
...
const asRevision = (value: unknown, key: string, nullable = false): RevisionId | "HEAD" | null => {
  ...
  throw new AgentDocxError("INVALID_ARGUMENT", `${key} must be HEAD or a sha256 revision ID`);
};
...
const manifestRelativePath = (cwd: string, manifestPath: string, path: string): string => {
  const candidate = relative(dirname(manifestPath), invocationPath(cwd, path)).split(sep).join("/");
  if (!candidate || candidate === ".." || candidate.startsWith("../") || isAbsolute(candidate))
    throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `Path is outside project: ${path}`);
  return candidate;
};
...
const projectInput = (
  ...
  const source = manifestRelativePath(cwd, manifestPath, requiredString(params, "source"));
  ...
  const chrome = params.chrome === undefined ? undefined : assertRecord(params.chrome, "chrome");
  let fontSet: ProjectDocumentInput["fontSet"];
  if (params.fontSet !== undefined) {
    const raw = assertRecord(params.fontSet, "fontSet");
    assertFontSet(raw);
    assertKeys(raw, ["family", "regularPath", "boldPath", "italicPath", "boldItalicPath"], "fontSet");
    const family = requiredString(raw, "family");
    const regularPath = manifestRelativePath(cwd, manifestPath, requiredString(raw, "regularPath"));
    ...
    fontSet = {
      family,
      regularPath,
      ...(boldPath ? { boldPath: manifestRelativePath(cwd, manifestPath, boldPath) } : {}),
      ...(italicPath ? { italicPath: manifestRelativePath(cwd, manifestPath, italicPath) } : {}),
      ...(boldItalicPath
        ? { boldItalicPath: manifestRelativePath(cwd, manifestPath, boldItalicPath) }
        : {}),
    };
  }
  return {
    ...
    ...(template ? { template: manifestRelativePath(cwd, manifestPath, template) } : {}),
    ...(assetsDir ? { assetsDir: manifestRelativePath(cwd, manifestPath, assetsDir) } : {}),
    ...(filingKind ? { filingKind: filingKind as ProjectDocumentInput["filingKind"] } : {}),
    ...(rulePack ? { rulePack: rulePack as ProjectDocumentInput["rulePack"] } : {}),
    ...(fontSet ? { fontSet } : {}),
...
const blockIdPattern = /^b_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
const annotationIdPattern = /^a_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
...
const rendererModes = ["deterministic", "word", "libreoffice", "compare"] as const;

const hasOwn = (value: Record<string, unknown>, key: string): boolean =>
  Object.prototype.hasOwnProperty.call(value, key);

const requiredDocumentId = (value: Record<string, unknown>, key: string): string => {
  const documentId = requiredString(value, key);
  if (!documentIdPattern.test(documentId))
    throw new AgentDocxError("INVALID_ARGUMENT", `${key} must be a document ID`);
  return documentId;
};
...
const optionalRevision = (
  ...
  value[key] === undefined ? undefined : (asRevision(value[key], key) as RevisionId | "HEAD");
...
const requiredText = (
  ...
  if (typeof value[key] !== "string")
    throw new AgentDocxError("INVALID_ARGUMENT", `${label}.${key} must be a string`);
  return value[key] as string;
};
...
const assertDate = (value: string, label: string): void => {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value))
    throw new AgentDocxError("INVALID_ARGUMENT", `${label} must use YYYY-MM-DD`);
  const parsed = new Date(`${value}T00:00:00.000Z`);
  if (Number.isNaN(parsed.valueOf()) || parsed.toISOString().slice(0, 10) !== value)
    throw new AgentDocxError("INVALID_ARGUMENT", `${label} must be a valid date`);
};
...
const assertMetadata = (value: unknown): void => {
  ...
  if (metadata.filingDate !== undefined)
    assertDate(requiredText(metadata, "filingDate", "metadata"), "metadata.filingDate");

  const parties = metadata.parties;
  if (!Array.isArray(parties))
    throw new AgentDocxError("INVALID_ARGUMENT", "metadata.parties must be an array");
  ...
  if (!Array.isArray(counsel))
    throw new AgentDocxError("INVALID_ARGUMENT", "metadata.counsel must be an array");
  ...
  for (const certificate of certificates) {
    ...
    if (record.kind === "service") {
      ...
      if (record.date !== undefined)
        assertDate(requiredText(record, "date", "service certificate"), "service certificate.date");
...
const assertChrome = (value: unknown): void => {
  ...
  assertKeys(chrome, ["headers", "footers", "pageNumber", "lineNumbers"], "chrome");
  ...
  if (chrome.pageNumber !== undefined) {
    ...
    requiredEnum(pageNumber, "format", ["decimal", "lower-roman", "upper-roman"]);
...
const assertRendererOptions = (
  ...
  if (options.libreoffice !== undefined) {
    const libreoffice = assertRecord(options.libreoffice, `${label}.libreoffice`);
    assertKeys(
      libreoffice,
    ...
    if (libreoffice.installedFonts !== undefined) {
      ...
      for (const font of libreoffice.installedFonts) {
        const installed = assertRecord(font, `${label}.libreoffice.installedFonts[]`);
        assertKeys(installed, ["family", "path"], `${label}.libreoffice.installedFonts[]`);
...
const assertConfigUpdate = (value: unknown): void => {
  ...
  if (Object.keys(changes).length === 0)
    throw new AgentDocxError("INVALID_ARGUMENT", "changes must include at least one property");
...
const assertSourcePatch = (value: unknown): void => {
  ...
  assertKeys(patch, ["schemaVersion", "documentId", "baseRevision", "edits"], "patch");
  ...
  for (const edit of patch.edits) {
    ...
    assertKeys(record, ["start", "end", "expectedText", "replacement"], "patch edit");
    requiredInteger(record, "start");
    requiredInteger(record, "end");
    if ((record.end as number) < (record.start as number))
      throw new AgentDocxError("PATCH_INVALID", "patch edit end must not precede start");
...
const assertChangeSet = (value: unknown): void => {
  ...
  if (changeSet.schemaVersion !== 1)
    throw new AgentDocxError("CHANGESET_INVALID", "changeSet.schemaVersion must be 1");
  ...
  if (!Array.isArray(changeSet.changes) || !Array.isArray(changeSet.annotations))
    throw new AgentDocxError(
      "CHANGESET_INVALID",
...
const assertResolutionDecisions = (value: unknown): void => {
  ...
  for (const [id, decision] of Object.entries(decisions)) {
    if (!changeIdPattern.test(id) || (decision !== "accept" && decision !== "reject"))
      throw new AgentDocxError(
        "CHANGESET_INVALID",
...
const assertReviewRange = (value: unknown): void => {
  ...
  if (end < start)
    throw new AgentDocxError("INVALID_ARGUMENT", "Review range end must not precede start");
};

const assertImportParams = (value: Record<string, unknown>): void => {
  assertKeys(
    value,
    ["input", "attachments", "inspectOnly", "documentId", "output", "author", "message"],
    "docx.import params",
  );
  requiredString(value, "input");
  optionalString(value, "attachments");
  if (typeof value.inspectOnly !== "boolean")
    throw new AgentDocxError("INVALID_ARGUMENT", "inspectOnly must be a boolean");
...
const assertAgentParams = (
  ...
  switch (action) {
    ...
    case "document.measure":
      assertKeys(params, ["documentId", "revision", "options"], "document.measure params");
      requiredDocumentId(params, "documentId");
      optionalRevision(params, "revision");
      if (params.options !== undefined)
        assertRendererOptions(
          params.options,
          "document.measure options",
          [
            "renderer",
            "officeTimeoutMs",
    ...
    case "revision.checkpoint":
      assertKeys(params, ["documentId", "baseRevision", "author", "message"], "revision.checkpoint params");
      requiredDocumentId(params, "documentId");
      asRevision(params.baseRevision, "baseRevision", true);
      actor(params.author);
      requiredString(params, "message");
      return;
    case "revision.list":
      assertKeys(params, ["documentId", "limit", "cursor"], "revision.list params");
      requiredDocumentId(params, "documentId");
    ...
    case "revision.restore":
      ...
      return;
    case "revision.diff":
      assertKeys(params, ["documentId", "baseRevision", "headRevision", "output"], "revision.diff params");
      requiredDocumentId(params, "documentId");
      asRevision(params.baseRevision, "baseRevision");
    ...
    case "draft.evaluate":
      ...
      return;
    case "draft.apply":
      assertKeys(params, ["patch", "patchHash", "gate", "author", "message"], "draft.apply params");
      assertSourcePatch(params.patch);
    ...
    case "review.add":
      ...
      requiredDocumentId(params, "documentId");
      asRevision(params.revision, "revision");
      if (typeof params.blockId !== "string" || !blockIdPattern.test(params.blockId))
        throw new AgentDocxError("INVALID_ARGUMENT", "blockId must be a block ID");
    ...
    case "review.resolve":
      ...
      requiredDocumentId(params, "documentId");
      asRevision(params.revision, "revision");
      if (typeof params.annotationId !== "string" || !annotationIdPattern.test(params.annotationId))
        throw new AgentDocxError("INVALID_ARGUMENT", "annotationId must be an annotation ID");
      actor(params.author);
      requiredString(params, "message");
      return;
    case "docx.export": {
      ...
      requiredDocumentId(params, "documentId");
      asRevision(params.revision, "revision");
      const mode = requiredEnum(params, "mode", ["clean", "redline"]);
      requiredString(params, "output");
      if (mode === "clean" && hasOwn(params, "baseRevision"))
        throw new AgentDocxError("INVALID_ARGUMENT", "Clean DOCX export forbids baseRevision");
...
export const parseAgentRequest = (value: unknown): AgentRequest => {
  ...
  assertKeys(request, ["schemaVersion", "id", "action", "project", "params"], "Agent request");
  if (request.schemaVersion !== 1)
    throw new AgentDocxError("INVALID_ARGUMENT", "Agent request schemaVersion must be 1");
  if (
    hasOwn(request, "id") &&
    request.id !== null &&
  ...
  if (
    ...
    throw new AgentDocxError("INVALID_ARGUMENT", "Agent request action is invalid");
  if (
  ...
  if (record) {
    ...
    const measurement =
      ...
      !Array.isArray(record.measurement)
        ...
        : null;
    const measuredRevision = measurement ? revisionId(measurement.revision) : null;
...
export const dispatchAgentRequest = async (
  ...
  switch (request.action) {
    ...
    case "project.add": {
      ...
      value = await project.addDocument(projectInput(cwd, manifestPath, request.params, true));
      break;
    }
    case "project.get": {
    ...
    case "document.configure": {
      ...
      const input: ConfigureDocumentInput = {
        baseRevision: asRevision(request.params.baseRevision, "baseRevision", true),
        changes,
        author: actor(request.params.author),
        message: requiredString(request.params, "message"),
      };
      value = await (await openProject(manifestPath)).configureDocument(
    ...
    case "document.get": {
      assertKeys(request.params, ["documentId", "revision"], "document.get params");
      const revision = request.params.revision === undefined ? undefined : asRevision(request.params.revision, "revision") as RevisionId | "HEAD";
      value = await (await openProject(manifestPath)).getDocument(requiredString(request.params, "documentId"), revision);
      break;
    }
    case "document.measure": {
      assertKeys(request.params, ["documentId", "revision", "options"], "document.measure params");
      const options = request.params.options === undefined ? {} : assertRecord(request.params.options, "options");
      const revision = request.params.revision === undefined ? undefined : asRevision(request.params.revision, "revision") as RevisionId | "HEAD";
      value = await (await openProject(manifestPath)).measure(requiredString(request.params, "documentId"), revision, options);
      break;
    }
    case "document.validate": {
      assertKeys(request.params, ["documentId", "revision"], "document.validate params");
      const revision = request.params.revision === undefined ? undefined : asRevision(request.params.revision, "revision") as RevisionId | "HEAD";
      value = await (await openProject(manifestPath)).validate(requiredString(request.params, "documentId"), revision);
      break;
    }
    case "revision.checkpoint": {
      assertKeys(request.params, ["documentId", "baseRevision", "author", "message"], "revision.checkpoint params");
      value = await (await openProject(manifestPath)).checkpoint(requiredString(request.params, "documentId"), {
        baseRevision: asRevision(request.params.baseRevision, "baseRevision", true),
        author: actor(request.params.author),
        message: requiredString(request.params, "message"),
      });
    ...
    case "revision.list": {
      ...
      value = await (await openProject(manifestPath)).listRevisions(
        requiredDocumentId(request.params, "documentId"),
        {
    ...
    case "revision.get": {
      assertKeys(request.params, ["documentId", "revision"], "revision.get params");
      value = await (await openProject(manifestPath)).getRevision(requiredString(request.params, "documentId"), asRevision(request.params.revision, "revision") as RevisionId | "HEAD");
      break;
    }
    case "revision.restore": {
      assertKeys(request.params, ["documentId", "baseRevision", "targetRevision", "author", "message"], "revision.restore params");
      value = await (await openProject(manifestPath)).restore(requiredString(request.params, "documentId"), {
        baseRevision: asRevision(request.params.baseRevision, "baseRevision") as RevisionId | "HEAD",
        targetRevision: asRevision(request.params.targetRevision, "targetRevision") as RevisionId | "HEAD",
        author: actor(request.params.author),
        message: requiredString(request.params, "message"),
      });
      break;
    ...
    case "revision.diff": {
      assertKeys(request.params, ["documentId", "baseRevision", "headRevision", "output"], "revision.diff params");
      const project = await openProject(manifestPath);
      const documentId = requiredString(request.params, "documentId");
      const baseRevision = asRevision(request.params.baseRevision, "baseRevision") as RevisionId | "HEAD";
      const headRevision = asRevision(request.params.headRevision, "headRevision") as RevisionId | "HEAD";
      const changeSet = await project.diff(documentId, baseRevision, headRevision);
    ...
    case "revision.resolve": {
      assertKeys(request.params, ["documentId", "changeSet", "decisions", "author", "message"], "revision.resolve params");
      const decisions = assertRecord(request.params.decisions, "decisions");
      for (const decision of Object.values(decisions))
        if (decision !== "accept" && decision !== "reject")
          throw new AgentDocxError("CHANGESET_INVALID", "Resolution decisions must be accept or reject");
      ...
      value = await (await openProject(manifestPath)).resolveChanges(requiredString(request.params, "documentId"), input);
      break;
    }
    case "draft.guidance": {
      assertKeys(request.params, ["documentId", "revision"], "draft.guidance params");
      const revision = request.params.revision === undefined ? undefined : asRevision(request.params.revision, "revision") as RevisionId | "HEAD";
      value = await (await openProject(manifestPath)).getDraftGuidance(requiredString(request.params, "documentId"), revision);
      break;
    }
    case "draft.evaluate": {
      assertKeys(request.params, ["patch", "renderer"], "draft.evaluate params");
      const renderer = optionalString(request.params, "renderer");
      value = await (await openProject(manifestPath)).evaluatePatch(asPatch(request.params.patch), renderer ? { renderer: renderer as "deterministic" | "word" | "libreoffice" | "compare" } : {});
      break;
    }
    case "draft.apply": {
      assertKeys(request.params, ["patch", "patchHash", "gate", "author", "message"], "draft.apply params");
      const gate = optionalString(request.params, "gate");
      value = await (await openProject(manifestPath)).applyPatch(asPatch(request.params.patch), {
    ...
    case "review.add": {
      assertKeys(request.params, ["documentId", "revision", "blockId", "range", "author", "message"], "review.add params");
      ...
      if (request.params.range !== undefined) {
        ...
        if (!Number.isInteger(raw.start) || !Number.isInteger(raw.end))
          throw new AgentDocxError("INVALID_ARGUMENT", "Review range must use integer offsets");
        range = { start: raw.start as number, end: raw.end as number };
      }
      value = await (await openProject(manifestPath)).addReview(requiredString(request.params, "documentId"), {
        revision: asRevision(request.params.revision, "revision") as RevisionId | "HEAD",
        blockId: requiredString(request.params, "blockId") as AddReviewInput["blockId"],
        ...(range ? { range } : {}),
        author: actor(request.params.author),
        message: requiredString(request.params, "message"),
      });
    ...
    case "review.resolve": {
      assertKeys(request.params, ["documentId", "revision", "annotationId", "author", "message"], "review.resolve params");
      const input: ResolveReviewInput = {
        revision: asRevision(request.params.revision, "revision") as RevisionId | "HEAD",
        annotationId: requiredString(request.params, "annotationId") as ResolveReviewInput["annotationId"],
        author: actor(request.params.author),
        message: requiredString(request.params, "message"),
      };
      value = await (await openProject(manifestPath)).resolveReview(requiredString(request.params, "documentId"), input);
      break;
    }
    case "docx.export": {
      assertKeys(request.params, ["documentId", "revision", "mode", "baseRevision", "output", "options"], "docx.export params");
      const mode = requiredString(request.params, "mode");
      const revision = asRevision(request.params.revision, "revision") as RevisionId | "HEAD";
      const output = invocationPath(cwd, requiredString(request.params, "output"));
      const options = request.params.options === undefined ? undefined : assertRecord(request.params.options, "options");
      let input: ExportDocxInput;
      if (mode === "clean") {
        if (request.params.baseRevision !== undefined)
          throw new AgentDocxError("INVALID_ARGUMENT", "Clean DOCX export forbids baseRevision");
        input = { revision, mode, output, ...(options ? { options } : {}) } as ExportDocxInput;
      ...
      } else throw new AgentDocxError("INVALID_ARGUMENT", "DOCX export mode must be clean or redline");
      value = await (await openProject(manifestPath)).exportDocx(requiredString(request.params, "documentId"), input);
      break;
    }
    case "docx.import": {
      assertKeys(request.params, ["input", "attachments", "inspectOnly", "documentId", "output", "author", "message"], "docx.import params");
      const inspectOnly = request.params.inspectOnly;
      if (typeof inspectOnly !== "boolean")
        throw new AgentDocxError("INVALID_ARGUMENT", "inspectOnly must be a boolean");
      const inputPath = invocationPath(cwd, requiredString(request.params, "input"));
      const attachmentPath = optionalString(request.params, "attachments");
      const attachments = attachmentPath ? { directory: invocationPath(cwd, attachmentPath) } : undefined;
      if (inspectOnly) {
        if (["documentId", "output", "author", "message"].some((key) => request.params[key] !== undefined))
          throw new AgentDocxError("INVALID_ARGUMENT", "Inspect-only DOCX import is stateless");
        value = await inspectDocx(inputPath, attachments ? { attachments } : {});
      } else {
    ...
    case "docx.inspect": {
      ...
      value = await inspectDocxTemplate(await readFile(invocationPath(cwd, requiredString(request.params, "input"))));
      break;
    }
  }
...
export const serializeAgentValue = (
  ...
  const serialize = (
    ...
    for (const [childKey, child] of Object.entries(
      ...
      if (childKey === "generatedDocx" || child instanceof Uint8Array)
        continue;
      if (
