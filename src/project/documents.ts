...
export const assertStoredDocument = (value: unknown, path: string): LegalDocument => {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    return storedObjectCorrupt(path);
  const record = value as Record<string, unknown>;
  if (
...
export const storedObjectCorrupt = (path: string): never => {
  throw new AgentDocxError("PROJECT_INVALID", `Stored object is corrupt: ${path}`);
};


export const measureSnapshot = async (_ctx: ProjectContext, 
    config: AgentDocxDocumentConfig,
    snapshot: ProjectSnapshot,
...
export const prepareWorkingDocument = async (_ctx: ProjectContext, 
    opened: OpenedStore,
    config: AgentDocxDocumentConfig,
    snapshot: ProjectSnapshot,
    annotations: readonly ReviewAnnotation[],
...
export const commitLocked = async (ctx: ProjectContext, 
    opened: OpenedStore,
    config: AgentDocxDocumentConfig,
    snapshot: ProjectSnapshot,
    document: LegalDocument,
    ...
    const parentMaterial = parent
      ? await materialFor(ctx, opened, parent)
      : null;
    const headParentMaterial =
      headParent && headParent.id === parent?.id
        ? parentMaterial
        : headParent
          ? await materialFor(ctx, opened, headParent)
          : null;
    ...
    const measurement = await measureSnapshot(ctx, 
      config,
      snapshot,
      committedDocument,
      revisionId,
...
export const getStateLocked = async (ctx: ProjectContext, opened: OpenedStore): Promise<ProjectState> => {
...
export const getDocumentLocked = async (ctx: ProjectContext, 
    opened: OpenedStore,
    documentId: string,
    ...
    if (revision !== undefined) {
      const record = await currentRevision(ctx, opened, documentId, revision);
    ...
    const working = await workingSnapshotLocked(ctx, opened, config, documentId);
    return {
      schemaVersion: 1,
      documentId,
      revision: null,
...
export const measureLocked = async (ctx: ProjectContext, 
    opened: OpenedStore,
    documentId: string,
    revision?: RevisionId | "HEAD",
    options: ProjectMeasureOptions = {},
    ...
    if (revision === undefined) {
      ...
      return measureSnapshot(ctx, config, working.snapshot, working.document, null, {
        ...options,
    ...
    const customPacks = await configuredRulePacks(opened, material.config, snapshot);
    return measureSnapshot(ctx, 
      material.config,
      snapshot,
      material.document,
      record.id,
...
export const workingSnapshotLocked = async (ctx: ProjectContext, 
    opened: OpenedStore,
    config: AgentDocxDocumentConfig,
    documentId: string,
  ): Promise<{
    ...
    const snapshot = await snapshotProjectDocument(opened, config);
    const headAnnotations = await annotationsForHead(ctx, opened, documentId);
...
export const getDocument = async (ctx: ProjectContext, 
    documentId: string,
    revision?: RevisionId | "HEAD",
  ): Promise<DocumentSnapshot> => {
    return withLockedStore(ctx.manifestPath, (opened) =>
      getDocumentLocked(ctx, opened, documentId, revision),
    );
  };

export const configureDocument = async (ctx: ProjectContext, 
    documentId: string,
    ...
    return withLockedStore(ctx.manifestPath, async (opened) => {
      ...
      const annotations = await annotationsForHead(ctx, opened, documentId);
      const prepared = await prepareWorkingDocument(ctx, 
        opened,
        next,
        snapshot,
        annotations,
...
export const checkpoint = async (ctx: ProjectContext, 
    documentId: string,
    input: {
...
export const measure = async (ctx: ProjectContext, 
    documentId: string,
    revision?: RevisionId | "HEAD",
    options: ProjectMeasureOptions = {},
  ): Promise<ProjectMeasurementResult> => {
    return withLockedStore(ctx.manifestPath, (opened) =>
      measureLocked(ctx, opened, documentId, revision, options),
    );
  };

export const validateLocked = async (ctx: ProjectContext, 
    opened: OpenedStore,
    documentId: string,
    revision?: RevisionId | "HEAD",
    ...
    if (revision === undefined) {
      ...
      selectedSnapshot = await snapshotForMaterial(ctx, opened, material);
      document = material.document;
...
export const validate = async (ctx: ProjectContext, 
    documentId: string,
    revision?: RevisionId | "HEAD",
  ): Promise<ValidationResult> => {
    return withLockedStore(ctx.manifestPath, (opened) =>
      validateLocked(ctx, opened, documentId, revision),
    );
  };

export const getDraftGuidance = async (ctx: ProjectContext, 
    documentId: string,
    revision?: RevisionId | "HEAD",
...
export const evaluatePatch = async (ctx: ProjectContext, 
    patch: SourcePatch,
    options: {
    ...
    try {
      ...
      const candidateFontSet = sourceFontSet(
        material.config,
        candidateSnapshot,
      );
...
export const applyPatch = async (ctx: ProjectContext, 
    patch: SourcePatch,
    input: {
      patchHash: string;
    ...
    return withLockedStore(ctx.manifestPath, async (opened) => {
      ...
      const [beforeMeasurement, candidateMeasurement] = await Promise.all([
        ...
        measureSnapshot(ctx, config, candidateSnapshot, document, record.id),
      ]);
