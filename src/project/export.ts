...
import { createChangeSet, defaultAttribution } from "../revisions/diff.js";
...
export const exportDocx = async (
  ...
  return withLockedStore(ctx.manifestPath, async (opened) => {
    ...
    const compiled = await compileMarkdown(
      ...
      {
        ...input.options,
        ...(input.mode === "pdf" ? { renderer: "deterministic" as const } : {}),
        rulePacks: await configuredRulePacks(opened, material.config, snapshot),
        generation: {
          revision: record,
    ...
    if (input.mode === "redline") {
      ...
      if (
        base.id === record.id ||
        !(await isFirstParentAncestor(opened, base.id, record))
      )
        throw new AgentDocxError(
    ...
    try {
      ...
      if (pdfBytes)
        await writeExclusiveFile(resolve(stagePath, "document.pdf"), pdfBytes);
