...
export type ProjectSnapshot = {
  ...
  dependencyBytes: ReadonlyMap<string, { bytes: Uint8Array; mediaType: string }>;
...
export const canonicalJson = (value: unknown): string => {
  ...
  if (serialized === undefined)
    throw new AgentDocxError("PROJECT_INVALID", "Value cannot be canonicalized");
  return serialized;
};
...
const assertRelativeManifestPath = (path: string, name: string): string => {
  if (
    ...
    throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `${name} must be a normalized project-relative path`);
  return path;
};

const assertWithin = (projectDirectory: string, path: string, name: string): string => {
  ...
  if (
    ...
    throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `${name} is outside the project directory`);
  return absolutePath;
};
...
const assertRegularFile = async (path: string, name: string): Promise<void> => {
  ...
  try {
    ...
    throw new AgentDocxError("INPUT_NOT_FOUND", `${name} does not exist: ${path}`);
  }
  if (!entry.isFile() || entry.isSymbolicLink())
    throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `${name} must be a regular nonsymlink file`);
};
...
const assertDirectory = async (path: string, name: string): Promise<void> => {
  ...
  if (!entry.isDirectory() || entry.isSymbolicLink())
    throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `${name} must be a directory, not a symlink`);
};
...
const hasOnlyKeys = (value: Record<string, unknown>, allowed: Record<string, true>): boolean =>
  Object.keys(value).every((key) => allowed[key] === true);

const objectRecord = (value: unknown, name: string): Record<string, unknown> => {
...
const assertClosedKeys = (
  ...
  if (!Object.keys(value).every((key) => keys.includes(key)))
    throw new AgentDocxError("PROJECT_INVALID", `${name} has unknown properties`);
};
...
const assertMetadata = (value: unknown): void => {
  ...
  for (const key of ["court", "jurisdiction", "caseName", "docketNumber", "documentTitle"] as const)
    assertString(metadata[key], `Document metadata ${key}`);
  if (
    metadata.filingDate !== undefined &&
    (typeof metadata.filingDate !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(metadata.filingDate))
  )
    throw new AgentDocxError("PROJECT_INVALID", "Document metadata filingDate is invalid");
  for (const [key, allowed] of [
    ["parties", ["id", "name", "role"]],
    ["counsel", ["id", "name", "barNumber", "firm", "addressLines", "phone", "email"]],
  ] as const) {
    if (!Array.isArray(metadata[key]))
      throw new AgentDocxError("PROJECT_INVALID", `Document metadata ${key} must be an array`);
    ...
    for (const entry of metadata[key]) {
      ...
      if (!isDocumentId(id) || ids.has(id))
        throw new AgentDocxError("PROJECT_INVALID", `Document metadata ${key} entry id is invalid`);
      ids.add(id);
      assertString(record.name, `Document metadata ${key} entry name`);
      if (key === "parties") assertString(record.role, "Document metadata party role");
      if (record.addressLines !== undefined) {
        if (!Array.isArray(record.addressLines) || record.addressLines.some((line) => typeof line !== "string"))
          throw new AgentDocxError("PROJECT_INVALID", "Document metadata counsel addressLines is invalid");
      }
      for (const optional of ["barNumber", "firm", "phone", "email"] as const)
        if (record[optional] !== undefined) assertString(record[optional], `Document metadata counsel ${optional}`);
    }
  }
  if (!Array.isArray(metadata.certificates))
    throw new AgentDocxError("PROJECT_INVALID", "Document metadata certificates must be an array");
  ...
  for (const certificate of metadata.certificates) {
    ...
    if (!isDocumentId(id) || certificateIds.has(id))
      throw new AgentDocxError("PROJECT_INVALID", "Document metadata certificate id is invalid");
    certificateIds.add(id);
    if (record.kind === "service") {
      assertClosedKeys(record, ["id", "kind", "statement", "servedOn", "method", "date", "signerCounselId"], "Service certificate");
      assertString(record.statement, "Service certificate statement");
      assertString(record.method, "Service certificate method");
      assertString(record.signerCounselId, "Service certificate signerCounselId");
      if (!Array.isArray(record.servedOn) || record.servedOn.some((entry) => typeof entry !== "string"))
        throw new AgentDocxError("PROJECT_INVALID", "Service certificate servedOn is invalid");
      if (record.date !== undefined) assertString(record.date, "Service certificate date");
    } else if (record.kind === "compliance") {
      assertClosedKeys(record, ["id", "kind", "basis", "signerCounselId"], "Compliance certificate");
      if (record.basis !== "words" && record.basis !== "monospaced-lines")
        throw new AgentDocxError("PROJECT_INVALID", "Compliance certificate basis is invalid");
      assertString(record.signerCounselId, "Compliance certificate signerCounselId");
    } else
      throw new AgentDocxError("PROJECT_INVALID", "Certificate kind is invalid");
  }
};

const assertChrome = (value: unknown): void => {
  const chrome = objectRecord(value, "Document chrome");
  assertClosedKeys(chrome, ["headers", "footers", "pageNumber", "lineNumbers"], "Document chrome");
  for (const storyKind of ["headers", "footers"] as const) {
    if (chrome[storyKind] === undefined) continue;
    const stories = objectRecord(chrome[storyKind], `Document chrome ${storyKind}`);
    assertClosedKeys(stories, ["default", "first", "even"], `Document chrome ${storyKind}`);
    for (const text of Object.values(stories)) {
      ...
      for (const token of template.matchAll(/\{\{([^}]+)\}\}/g))
        if (!["caseName", "docketNumber", "documentTitle", "page", "pages"].includes(token[1]!))
          throw new AgentDocxError("PROJECT_INVALID", `Unknown document chrome token: ${token[1]!}`);
    }
  }
  if (chrome.pageNumber !== undefined) {
    const number = objectRecord(chrome.pageNumber, "Document chrome pageNumber");
    assertClosedKeys(number, ["story", "alignment", "format", "start"], "Document chrome pageNumber");
    if (number.story !== "header" && number.story !== "footer")
      throw new AgentDocxError("PROJECT_INVALID", "Document chrome pageNumber story is invalid");
    if (!["left", "center", "right"].includes(String(number.alignment)))
      throw new AgentDocxError("PROJECT_INVALID", "Document chrome pageNumber alignment is invalid");
    if (!["decimal", "lower-roman", "upper-roman"].includes(String(number.format)))
      throw new AgentDocxError("PROJECT_INVALID", "Document chrome pageNumber format is invalid");
    if (!Number.isInteger(number.start) || (number.start as number) < 1)
      throw new AgentDocxError("PROJECT_INVALID", "Document chrome pageNumber start is invalid");
  }
  if (chrome.lineNumbers !== undefined) {
    const lines = objectRecord(chrome.lineNumbers, "Document chrome lineNumbers");
    assertClosedKeys(lines, ["countBy", "start", "distanceTwips", "restart"], "Document chrome lineNumbers");
    for (const numeric of ["countBy", "start", "distanceTwips"] as const)
      if (!Number.isInteger(lines[numeric]) || (lines[numeric] as number) < (numeric === "distanceTwips" ? 0 : 1))
        throw new AgentDocxError("PROJECT_INVALID", `Document chrome lineNumbers ${numeric} is invalid`);
    if (!["continuous", "new-page", "new-section"].includes(String(lines.restart)))
      throw new AgentDocxError("PROJECT_INVALID", "Document chrome lineNumbers restart is invalid");
  }
};

const validateDocumentConfig = (value: unknown): AgentDocxDocumentConfig => {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new AgentDocxError("PROJECT_INVALID", "Document config must be an object");
  const config = value as Record<string, unknown>;
  if (!hasOnlyKeys(config, allowedDocumentKeys))
    throw new AgentDocxError("PROJECT_INVALID", "Document config has unknown properties");
  ...
  if (typeof config.profile !== "string" || !(config.profile in builtInProfiles))
  ...
  if (
    ...
    !["principal-brief", "reply-brief", "motion-document", "opposition-text", "reply-text"].includes(
      String(config.filingKind),
    )
  )
    throw new AgentDocxError("PROJECT_INVALID", "Document filingKind is invalid");
  if (
  ...
  if (config.fontSet !== undefined) {
    if (!config.fontSet || typeof config.fontSet !== "object" || Array.isArray(config.fontSet))
    ...
    if (typeof fontSet.regularPath !== "string")
      throw new AgentDocxError("PROJECT_INVALID", "fontSet regularPath is invalid");
    for (const role of ["regularPath", "boldPath", "italicPath", "boldItalicPath"] as const) {
      const path = fontSet[role];
      if (path !== undefined) {
        if (typeof path !== "string")
          throw new AgentDocxError("PROJECT_INVALID", `fontSet ${role} is invalid`);
...
export const validateManifest = (value: unknown): AgentDocxManifest => {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new AgentDocxError("PROJECT_INVALID", "Project manifest must be an object");
  const manifest = value as Record<string, unknown>;
  if (!hasOnlyKeys(manifest, allowedManifestKeys))
    throw new AgentDocxError("PROJECT_INVALID", "Project manifest has unknown properties");
  if (manifest.schemaVersion !== 1)
    throw new AgentDocxError("PROJECT_INVALID", "Project manifest schemaVersion must be 1");
  if (
  ...
  if (typeof manifest.defaultDocument !== "string" || !isDocumentId(manifest.defaultDocument))
    throw new AgentDocxError("PROJECT_INVALID", "Default document id is invalid");
  if (manifest.storeDir !== STORE_DIR)
    throw new AgentDocxError("PROJECT_INVALID", `Project storeDir must be ${STORE_DIR}`);
  if (!Array.isArray(manifest.documents) || manifest.documents.length === 0)
    throw new AgentDocxError("PROJECT_INVALID", "Project must contain at least one document");
  const documents = manifest.documents.map(validateDocumentConfig);
  const ids = new Set<string>();
  const paths = new Set<string>();
  for (const document of documents) {
    if (ids.has(document.id))
      throw new AgentDocxError("PROJECT_INVALID", `Duplicate document id: ${document.id}`);
    if (paths.has(document.source))
      throw new AgentDocxError("PROJECT_INVALID", `Duplicate document source: ${document.source}`);
    ids.add(document.id);
    paths.add(document.source);
  }
  if (!ids.has(manifest.defaultDocument))
    throw new AgentDocxError("PROJECT_INVALID", "Default document is not declared");
  return {
    schemaVersion: 1,
...
const readJsonFile = async <T>(path: string): Promise<T> => strictJson<T>(await readFile(path), path);
...
export const readObject = async (storePath: string, id: RevisionId): Promise<Uint8Array> => {
  const path = objectPath(storePath, id);
...
const refsPath = (storePath: string, documentId: string): string => {
  if (!isDocumentId(documentId))
    throw new AgentDocxError("DOCUMENT_NOT_FOUND", `Invalid document id: ${documentId}`);
...
const revisionPath = (storePath: string, id: RevisionId): string => {
  const hex = id.slice("sha256:".length);
  if (!/^[0-9a-f]{64}$/.test(hex))
    throw new AgentDocxError("REVISION_NOT_FOUND", `Invalid revision id: ${id}`);
...
export const readHead = async (
  ...
  try {
    const entry = await readJsonFile<{ schemaVersion: 1; documentId: string; head: RevisionId | null }>(path);
    if (entry.schemaVersion !== 1 || entry.documentId !== documentId)
      throw new AgentDocxError("PROJECT_INVALID", `Malformed head reference: ${documentId}`);
...
export const writeHead = async (
  ...
  await writeExclusive(stage, canonicalJson({ schemaVersion: 1, documentId, head }));
...
export const readRevisionJson = async <T>(
  ...
  const record = strictJson<Record<string, unknown>>(await readFile(path), path);
  const withoutId = { ...record };
  delete withoutId.id;
  if (record.id !== revision || canonicalObjectId(withoutId) !== revision)
    throw new AgentDocxError("PROJECT_INVALID", `Revision hash mismatch: ${revision}`);
  return record as T;
};
...
export const writeRevisionJson = async (
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error;
    const existing = strictJson<Record<string, unknown>>(await readFile(path), path);
    const existingWithoutId = { ...existing };
    delete existingWithoutId.id;
    if (existing.id !== id || canonicalObjectId(existingWithoutId) !== id)
      throw new AgentDocxError("PROJECT_INVALID", `Revision collision at ${id}`);
  }
  return id;
};
...
const collectAssets = async (
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    const logical = relativeDirectory ? `${relativeDirectory}/${entry.name}` : entry.name;
    const path = resolve(directory, entry.name);
    if (entry.isSymbolicLink())
      throw new AgentDocxError("PATH_OUTSIDE_PROJECT", `Asset is a symlink: ${logical}`);
    if (entry.isDirectory()) {
      for (const [key, value] of await collectAssets(path, logical)) assets.set(key, value);
      continue;
    }
    if (!entry.isFile())
      throw new AgentDocxError("REFERENCE_INVALID", `Unsupported asset entry: ${logical}`);
    assets.set(logical, { bytes: await readFile(path), mediaType: mediaTypeFor(logical) });
  }
  return assets;
};
...
export const snapshotProjectDocument = async (
  ...
  const sourcePath = assertWithin(opened.projectDirectory, document.source, "Document source");
  ...
  try {
    ...
    throw new AgentDocxError("INPUT_NOT_UTF8", `Document source is not UTF-8: ${document.source}`);
  }
  const dependencies = new Map<string, { bytes: Uint8Array; mediaType: string }>();
  const dependencyObjects: Record<string, RevisionId> = {};
  const profile = builtInProfiles[document.profile];
  if (!profile)
    throw new AgentDocxError("PROJECT_INVALID", `Unknown profile: ${document.profile}`);
  ...
  if (document.rulePack) {
    ...
    if (!pack)
      throw new AgentDocxError("PROJECT_INVALID", `Unknown rule pack: ${document.rulePack}`);
    ...
    dependencyObjects[`rule-source/${pack.sourceExcerpt}`] = await addDependency(
      source,
      "text/plain",
  ...
  if (document.template) {
    const templatePath = assertWithin(opened.projectDirectory, document.template, "Template");
  ...
  if (document.assetsDir) {
    const assetsPath = assertWithin(opened.projectDirectory, document.assetsDir, "Assets directory");
  ...
  if (document.fontSet) {
    ...
    for (const [role, configuredPath] of Object.entries(roles)) {
      ...
      const fontPath = assertWithin(opened.projectDirectory, configuredPath, `Font ${role}`);
  ...
  const sortedDependencies = Object.fromEntries(
    Object.entries(dependencyObjects).sort(([left], [right]) => left.localeCompare(right)),
  ) as DependencyHashes;
...
const bindingPath = (storePath: string): string => resolve(storePath, BINDING_FILE);

const validateBinding = async (opened: OpenedStore): Promise<void> => {
  const binding = await readJsonFile<ProjectBinding>(bindingPath(opened.storePath));
  if (
    binding.schemaVersion !== 1 ||
    binding.projectId !== opened.manifest.projectId ||
    binding.manifestBasename !== basename(opened.manifestPath)
  )
    throw new AgentDocxError("PROJECT_INVALID", "Manifest does not own this project store");
};
...
const recoverInitialization = async (
  ...
  if (
    ...
    throw new AgentDocxError("PROJECT_INVALID", "Malformed initialization intent");
  ...
  const binding = await readJsonFile<ProjectBinding>(bindingPath(storePath)).catch(() => null);
  if (binding?.projectId === intent.projectId) await rm(storePath, { recursive: true, force: true });
  await rm(intentPath, { force: true });
};
...
const intentPathValue = (
  ...
  if (typeof value !== "string" || !isAbsolute(value))
    throw new AgentDocxError("PROJECT_INVALID", `Export intent ${label} is invalid`);
...
const exportIntentFrom = (
  ...
  if (manifestPath !== resolve(projectDirectory, basename(manifestPath)))
    throw new AgentDocxError("PROJECT_INVALID", "Export intent manifest path is invalid");
  const outputPath = intentPathValue(projectDirectory, intent.outputPath, "outputPath");
  const attachmentPath =
    intent.attachmentPath === null
      ? null
      : intentPathValue(projectDirectory, intent.attachmentPath, "attachmentPath");
  const stagePath = intentPathValue(projectDirectory, intent.stagePath, "stagePath");
  ...
  if (
    ...
    throw new AgentDocxError("PROJECT_INVALID", "Malformed export intent paths");
  return {
    schemaVersion: 1,
    state,
...
const stageMarker = (stagePath: string): string => resolve(stagePath, "owner.json");
...
const ownedStage = async (
  ...
  if (!entry.isDirectory() || entry.isSymbolicLink())
    throw new AgentDocxError("PROJECT_INVALID", `Export stage is not a directory: ${stagePath}`);
  try {
...
const removeOwnedStage = async (intent: ExportIntent): Promise<void> => {
  if (await ownedStage(intent.stagePath, intent.owner, intent.projectId, intent.manifestPath))
    await rm(intent.stagePath, { recursive: true, force: true });
};

const hashRegularFile = async (path: string, label: string): Promise<RevisionId> => {
...
const attachmentPayloadPath = (value: unknown): string => {
  if (
    ...
    value.includes("\\") ||
    value.split("/").some((part, index) => index === 0
      ? part !== "files"
      : part.length === 0 || part === "." || part === "..")
  )
    throw new AgentDocxError("PROJECT_INVALID", "Attachment manifest payload path is invalid");
  return value;
};
...
const verifyAttachmentDirectory = async (
  ...
  if (objectId(manifestBytes) !== manifestSha256)
    throw new AgentDocxError("PROJECT_INVALID", "Published attachment manifest hash mismatch");
  ...
  if (
    ...
    throw new AgentDocxError("PROJECT_INVALID", "Published attachment manifest is invalid");
  const names = new Set<string>();
  for (const entry of manifest.entries) {
    if (names.has(entry.name)) throw new AgentDocxError("PROJECT_INVALID", "Duplicate attachment name");
...
const publishStagedDirectory = async (
  ...
  const visit = async (source: string, target: string): Promise<void> => {
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (entry.isSymbolicLink())
        throw new AgentDocxError("PROJECT_INVALID", `Export stage contains a symlink: ${sourcePath}`);
      if (entry.isDirectory()) {
        ...
        throw new AgentDocxError("PROJECT_INVALID", `Export stage contains an unsupported entry: ${sourcePath}`);
      }
    }
  };
  try {
...
const recoverExport = async (
  ...
  if (intent.manifestPath !== manifestPath)
    throw new AgentDocxError("PROJECT_INVALID", "Export intent belongs to another manifest");
  const manifest = validateManifest(await readJsonFile<unknown>(manifestPath));
  if (manifest.projectId !== intent.projectId)
    throw new AgentDocxError("PROJECT_INVALID", "Export intent belongs to another project");
  ...
  if (intent.state === "preparing") {
    if (await pathExists(intent.outputPath) || (intent.attachmentPath !== null && await pathExists(intent.attachmentPath)))
      throw new AgentDocxError(
        "PROJECT_INVALID",
  ...
  if (outputExists) {
    if ((await hashRegularFile(intent.outputPath, "Published DOCX")) !== intent.docxSha256)
      throw new AgentDocxError("PROJECT_INVALID", "Published DOCX hash does not match export intent");
  } else {
    if (!stageIsOwned || !(await pathExists(intent.docxStagePath)))
      throw new AgentDocxError("PROJECT_INVALID", "Prepared export is missing its DOCX stage");
    if ((await hashRegularFile(intent.docxStagePath, "Staged DOCX")) !== intent.docxSha256)
      throw new AgentDocxError("PROJECT_INVALID", "Staged DOCX hash does not match export intent");
    await link(intent.docxStagePath, intent.outputPath);
  }
  if (intent.attachmentPath !== null) {
    if (intent.attachmentManifestSha256 === null || intent.attachmentStagePath === null)
      throw new AgentDocxError("PROJECT_INVALID", "Prepared attachment export is incomplete");
    if (!(await pathExists(intent.attachmentPath))) {
      if (!stageIsOwned || !(await pathExists(intent.attachmentStagePath)))
        throw new AgentDocxError("PROJECT_INVALID", "Prepared export is missing its attachment stage");
      await publishStagedDirectory(intent.attachmentStagePath, intent.attachmentPath);
    }
  ...
  if (stageIsOwned) await rm(intent.stagePath, { recursive: true, force: true });
  await rm(intentPath, { force: true });
};
...
export const clearExportIntent = async (projectDirectory: string): Promise<void> => {
...
export const openStore = async (manifestPath: string): Promise<OpenedStore> => {
  ...
  try {
    ...
    const manifest = validateManifest(await readJsonFile<unknown>(absoluteManifestPath));
    const opened = {
    ...
    if ((error as AgentDocxError).code === "INPUT_NOT_FOUND")
      throw new AgentDocxError("PROJECT_NOT_FOUND", `Project not found: ${absoluteManifestPath}`);
    throw error;
  } finally {
    await release();
  }
...
export const initializeStore = async (
  ...
  if (
    ...
    throw new AgentDocxError("INVALID_ARGUMENT", "Manifest basename is reserved");
  const release = await acquireProjectLock(projectDirectory);
  try {
    await recoverInitialization(projectDirectory, absoluteManifestPath);
    ...
    try {
      ...
      throw new AgentDocxError("DOCUMENT_EXISTS", `Manifest already exists: ${absoluteManifestPath}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    try {
      await lstat(storePath);
      throw new AgentDocxError("DOCUMENT_EXISTS", `Project store already exists: ${storePath}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    const intentPath = resolve(projectDirectory, INIT_INTENT);
...
export const documentConfigFromInput = async (
  ...
  if (!isDocumentId(input.documentId))
    throw new AgentDocxError("INVALID_ARGUMENT", "document id must be a lowercase slug");
  const normalize = async (path: string, name: string): Promise<string> => {
    ...
    return assertRelativeManifestPath(relativePath(projectDirectory, absolutePath), name);
  };
  const sourcePath = assertWithin(projectDirectory, input.source, "Document source");
  if (input.createSource) {
    try {
      await lstat(sourcePath);
      throw new AgentDocxError("OUTPUT_EXISTS", `Source already exists: ${input.source}`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
  } else {
  ...
  const source = assertRelativeManifestPath(relativePath(projectDirectory, sourcePath), "Document source");
  const fontSet = input.fontSet
    ? {
        ...
        ...(input.fontSet.italicPath
          ? { italicPath: await normalize(input.fontSet.italicPath, "Font italic") }
          : {}),
    ...
    : undefined;
  if ((input.fontSet?.family === undefined) !== (input.fontSet?.regularPath === undefined))
    throw new AgentDocxError("INVALID_ARGUMENT", "Font family and regular font are paired");
  let assetsDir: string | undefined;
  if (input.assetsDir) {
    const absoluteAssets = assertWithin(projectDirectory, input.assetsDir, "Assets directory");
...
export const replaceOwnedFile = async (
  ...
  if (expectedOld === null) {
    ...
    if (objectId(await readFile(path)) !== expectedOld)
      throw new AgentDocxError("WORKING_COPY_CONFLICT", `Owned file changed: ${path}`);
  }
  const stage = `${path}.${randomUUID()}.stage`;
  ...
  try {
    if (expectedOld === null) {
      ...
      if (objectId(await readFile(backup)) !== expectedOld)
        throw new AgentDocxError("PROJECT_INVALID", `Owned backup mismatch: ${path}`);
      await link(stage, path);
    }
    if (objectId(await readFile(path)) !== nextId)
      throw new AgentDocxError("PROJECT_INVALID", `Owned replacement mismatch: ${path}`);
    if (expectedOld !== null) await rm(backup, { force: true });
    return nextId;
  } finally {
