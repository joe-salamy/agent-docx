...
export const importDocx = async (
  ...
  return withLockedStore(ctx.manifestPath, async (opened) => {
    ...
    try {
      ...
      const mutation = await commitLocked(
        ctx,
        opened,
        config,
        refreshed,
...
export const importRedline = async (
  ctx: ProjectContext,
  input: {
    documentId: string;
  ...
  return withLockedStore(ctx.manifestPath, async (opened) => {
    ...
    const head = await currentRevision(opened, config.id, semanticRevision);
    const base = await currentRevision(opened, config.id, semanticBaseRevision);
    if (
      base.id === head.id ||
    ...
    if (inspected.resolution === "complete") {
      ...
      const reviewerAnnotations = new Map(
        inspected.annotations.map((annotation) => [annotation.id, annotation]),
      );
