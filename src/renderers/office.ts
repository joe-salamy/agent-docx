...
async function run(
  ...
  child.stdout.on("data", (b: Buffer) => {
    out += b.length;
    if (out <= 1024 * 1024) chunksOut.push(b);
  });
  child.stderr.on("data", (b: Buffer) => {
    err += b.length;
    if (err <= 1024 * 1024) chunksErr.push(b);
  });
...
export async function resolveLibreOffice(explicit?: string): Promise<string> {
  if (explicit) {
    if (!explicit.startsWith("/") && process.platform !== "win32")
...
export async function renderLibreOffice(
  ...
  const root = await mkdtemp(join(tmpdir(), "md-page-count-lo-"));
...
export async function renderWord(
  ...
  if (response.timedOut)
    throw new MdPageCountError("WORD_TIMEOUT", `Word exceeded ${timeoutMs} ms`);
  ...
  if (
    ...
    throw new MdPageCountError(
      "WORD_RENDER_FAILED",
      "Word adapter did not return a result",
  ...
  return {
    ...
    build: value.build,
    activePrinter: value.activePrinter,
    requestedFontFamilies,
    generatedDocxSha256: sha(docx),
    durationMs: performance.now() - start,
