...
async function libraryBench() {
  ...
  console.log(`library first (cold font parse): ${(t1 - t0).toFixed(1)} ms pages=${r.pageCount}`);
  t0 = performance.now();
  for (let i = 0; i < 15; i++) {
  ...
  t1 = performance.now();
  console.log(`library 15× amortized: ${(t1 - t0).toFixed(1)} ms avg=${((t1 - t0) / 15).toFixed(1)} ms`);
  t0 = performance.now();
  for (let i = 0; i < 15; i++) {
    await measureMarkdown(mdBase + ` ${i} `.repeat(20), opts);
  }
  t1 = performance.now();
  console.log(`library 15× second loop (warm): ${(t1 - t0).toFixed(1)} ms avg=${((t1 - t0) / 15).toFixed(1)} ms`);
}
...
function cliBench() {
  ...
  let t0 = performance.now();
  const out = execSync(`printf '%s' '${requests.replace(/'/g, "'\\''")}' | node ./dist/cli.js measure --batch --input-jsonl`, {
    encoding: "utf8",
  });
  let t1 = performance.now();
  const lines = out.trim().split("\n").filter(Boolean).length;
  console.log(`CLI batch-jsonl 15× one spawn: ${(t1 - t0).toFixed(1)} ms results=${lines}`);
  execSync(`printf '# Hello\\n\\nLorem\\n' > /tmp/bench-measure.md`);
  t0 = performance.now();
  for (let i = 0; i < 5; i++) {
    execSync(`node ./dist/cli.js measure /tmp/bench-measure.md --json > /dev/null`);
  }
  t1 = performance.now();
  console.log(`CLI spawn-per-measure 5×: ${(t1 - t0).toFixed(1)} ms avg=${((t1 - t0) / 5).toFixed(1)} ms`);
}
